...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "Almortagel_12" or message.new_chat_members[0].username == "S_E_M_o123":
      try:
        ...
        pass
      return await message.reply_text(f"**انضم المرتجل الكبير  الي هنا الان [.](https://t.me/Almortagel_12)⚡**\n\n**يرجي من الاعضاء احترام وجوده 🥷**")
    dev = await get_dev(bot_username)
...
"عليه الطلاق من بنت الحلال\n دي @Almortagel_12 الكلام دا محصلش",
...
@Client.on_message(filters.command(["المرتجل","احمد المرتجل","المرتجل"], ""))
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["Almortagel_12"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["Almortagel_12"]:
   ...
   if message.from_user.username == "Almortagel_12":
     await message.reply_text(f"**حدثتلك تويت ي المرتجل باشا **")
   else:
...
async def tiillllli(client, message):
 if message.from_user.username in ["Almortagel_12"]:
   ...
   if message.from_user.username == "Almortagel_12":
     await message.reply_text(f"**حدثتلك صراحه ي المرتجل باشا **")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="➧ 𝙅𝙊𝙄𝙉 |⌯ ˼ @AlmortagelTech ˹🐉˼")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="➧ 𝙅𝙊𝙄𝙉 |⌯ ˼ @AlmortagelTech ˹🐉˼")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="➧ 𝙅𝙊𝙄𝙉 |⌯ ˼ @AlmortagelTech ˹🐉˼")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["Almortagel_12"]:
         await message.reply_text("**انتا المطور المرتجل يجدع 🥷**")
         return
     if userr.id == dev:
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton(
                        "ᥴ𝗁ꪀꪀᥱᥣ 𝗔𝗹𝗺𝗼𝗿𝘁𝗮𝗴𝗲𝗹 ⌯", url=f"https://t.me/AlmortagelTech"),
                ],
                [    
                    InlineKeyboardButton(  
